...
# Path to the folder where app.py is located
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Read the CSV from the same folder
csv_path = os.path.join(BASE_DIR, "diabetes.csv.csv")   # OR "diabetes.csv" if renamed
...
def app():
    ...
    # show image (put diabetes_image.jpeg in the same folder as app.py)
    try:
        img = Image.open("diabetes.jpg")  # change name if needed
        img = img.resize((200, 200))
        st.image(img, caption="Diabetes Image", width=200)
    except FileNotFoundError:
        st.info("Image file not found. Make sure 'diabetes_image.jpeg' is in the app folder.")
